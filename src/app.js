...
const VERSION = "0.2.1"; // bump on release
...
let bennyImagesUsedThisLesson = new Set();

function pickBennyImage() {
  const available = BENNY_IMAGES.filter(src => !bennyImagesUsedThisLesson.has(src));
  const pool = available.length > 0 ? available : BENNY_IMAGES;
  if (pool.length === BENNY_IMAGES.length) {
    bennyImagesUsedThisLesson.clear();
  }
  const chosen = pool[Math.floor(Math.random() * pool.length)];
  bennyImagesUsedThisLesson.add(chosen);
  return chosen;
}
const lessonEngine = window.lessonEngine;

const THEORY_MAP = {
  reflexive_verbs: () => (window.CzechGrammarTrainerContent || {}).reflexiveVerbsTheory || "",
  similar_words: () => (window.CzechGrammarTrainerContent || {}).similarWordsTheory || ""
};
...
const elements = {
  ...
  viewTheory: document.getElementById("viewTheory"),
  ...
  theoryBackButton: document.getElementById("theoryBackButton")
};
...
elements.theoryBackButton.addEventListener("click", () => showView("home"));

elements.topicSelector.addEventListener("click", (e) => {
  const link = e.target.closest("[data-theory-topic]");
  if (!link) return;
  e.preventDefault();
  e.stopPropagation();
  showTheory(link.dataset.theoryTopic);
});
...
function renderTopicSelector() {
  ...
  elements.topicSelector.innerHTML = TOPICS.map(topic => {
    const hasTheory = THEORY_MAP[topic.id];
    const theoryLink = hasTheory
      ? ` <a href="#" class="topic-theory-link" data-theory-topic="${topic.id}" aria-label="Teorie: ${escapeHtml(topic.label)}">Teorie</a>`
      : "";
    return `
...
function showTheory(topicId) {
  ...
  if (!topic) return;
  const getTheory = THEORY_MAP[topicId];
  if (!getTheory) return;
  const html = getTheory();
  if (!html) return;
  elements.theoryTitle.textContent = topic.label;
  elements.theoryContent.innerHTML = html;
  showView("theory");
}
...
function showFeedback(isCorrect, options = {}) {
  ...
  const bennyHintBlock = hint
    ? `<div class="feedback-hint">
        <img src="${pickBennyImage()}" alt="" class="feedback-dog" aria-hidden="true">
        <p class="feedback-hint-bubble"><strong>Hint:</strong> ${escapeHtml(hint)}</p>
      </div>`
    : "";
