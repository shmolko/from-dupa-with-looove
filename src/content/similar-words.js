(function () {
  ...
  const similarWordsGlossary = [
    { cz: ["uklidit", "uklidnit"], en: ["to tidy", "to calm"] },
    { cz: ["tip", "typ"], en: ["tip, recommendation", "type, kind"] },
    { cz: ["mýt", "mít"], en: ["to wash", "to have"] },
    { cz: ["být", "bít"], en: ["to be", "to hit"] },
    { cz: ["radit", "řadit"], en: ["to advise", "to sort"] },
    { cz: ["holka", "hůlka"], en: ["girl", "stick, wand"] },
    { cz: ["horký", "hořký"], en: ["hot", "bitter"] }
  ];

  function buildSimilarWordsTheory() {
    return `
<h3>Slovníček – anglické překlady</h3>
<ul class="theory-glossary">
${similarWordsGlossary.map(pair =>
  `  <li><code>${pair.cz[0]}</code> = ${pair.en[0]} &nbsp;|&nbsp; <code>${pair.cz[1]}</code> = ${pair.en[1]}</li>`
).join("\n")}
</ul>
`;
  }
  ...
  window.CzechGrammarTrainerContent.similarWordsTheory = buildSimilarWordsTheory();
}());
